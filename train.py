...
parser.add_argument(
    "--lr_decay", type=eval, default=config["train"]["lr_decay"], help="是否开启初始学习率衰减策略"
)
parser.add_argument(
    "--lr_decay_step",
    type=str,
...
parser.add_argument(
    "--epochs", type=int, default=config["train"]["epochs"], help="训练代数"
)
parser.add_argument(
    "--print_every",
    type=int,
    default=config["train"]["print_every"],
    help="几个batch报训练损失",
)
...
parser.add_argument(
    "--max_grad_norm", type=float, default=config["train"]["max_grad_norm"], help="梯度阈值"
)

parser.add_argument(
    "--patience", type=int, default=config["train"]["patience"], help="等待代数"
)
...
def main():
    ...
    log_string(
        log, "The shape of localized adjacency matrix: {}".format(local_adj.shape)
    )

    log_string(
        log,
        f'trainX: {torch.tensor(dataloader["train_loader"].xs).shape}\t\t '
        f'trainY: {torch.tensor(dataloader["train_loader"].ys).shape}',
    )
    log_string(
        log,
        f'valX:   {torch.tensor(dataloader["val_loader"].xs).shape}\t\t'
        f'valY:   {torch.tensor(dataloader["val_loader"].ys).shape}',
    )
    log_string(
        log,
        f'testX:   {torch.tensor(dataloader["test_loader"].xs).shape}\t\t'
    ...
    # 开始训练
    log_string(log, f"compiling model... {DATASET}")
    his_loss = []
    val_time = []
    train_time = []
    ...
    formatted_date = time_now.strftime("%Y_%m_%d_%H_%M_%S")

    save_folder = config["test"]["log_folder"]
    save_csv_path = os.path.join(save_folder, f"{formatted_date}_STSGCN_{DATASET}_metric.csv")
    save_npz_path = os.path.join(save_folder, f"{formatted_date}_STSGCN_{DATASET}_predictions.npz")
